...
class ServerManager
{
    ...
    async startGameServerInstance()
    {
        ...
        if(1 === Number(process.env.RELDENS_CREATE_CONFIG_FILE || 1)){
            ...
            // Ensure the generated client config uses the public URL instead of localhost (cloud deployments):
            let generatedConfigFilePath = FileHandler.joinPaths(this.themeManager.projectThemePath, 'config.js');
            let generatedConfigContent = FileHandler.readFile(generatedConfigFilePath);
            let publicUrl = String(process.env.RELDENS_PUBLIC_URL || '').replace(/\/+$/, '');
            if(generatedConfigContent && publicUrl){
                let updatedConfigContent = generatedConfigContent
                    .split('http://localhost:'+this.configServer.port).join(publicUrl)
                    .split('http://localhost:8080').join(publicUrl);
                if(updatedConfigContent !== generatedConfigContent){
                    FileHandler.writeFile(generatedConfigFilePath, updatedConfigContent);
                    Logger.info('Client config server URLs updated with public URL: '+publicUrl);
                }
            }
